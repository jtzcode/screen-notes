...
export async function postToX(options: XBrowserOptions): Promise<void> {
  ...
  try {
    ...
    if (text) {
      ...
      // Select all + backspace to avoid appending to stale draft content.
      ...
      const insertedText = await cdp.send<{ result: { value: string } }>('Runtime.evaluate', {
        expression: `(document.querySelector('[data-testid="tweetTextarea_0"]')?.innerText || '').trim()`,
        returnByValue: true,
      }, { sessionId });

      if (!insertedText.result.value) {
        console.warn('[x-browser] Input.insertText produced empty editor. Falling back to execCommand insertText...');
        const fallbackText = await cdp.send<{ result: { value: string } }>('Runtime.evaluate', {
          expression: `
            (() => {
        ...
        if (!fallbackText.result.value) {
          throw new Error('Failed to insert text into X composer.');
        }
      }
    }
